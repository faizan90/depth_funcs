...
cpdef np.ndarray depth_ftn_mp(const DT_D_NP_t[:, :] x, 
    ...
    for i in prange(n_ei, schedule='dynamic', nogil=True, num_threads=n_cpus):
        tid = threadid()
...
# cpdef np.ndarray depth_ftn_mp_2(const DT_D_NP_t[:, :] x, 
#                               const DT_D_NP_t[:, :] y, 
#                               const DT_D_NP_t[:, :] ei,
#                               DT_UL n_cpus=1):
#     
#     cdef:
#         Py_ssize_t i, j, k
#         DT_UL n_mins, n_ei, tid, _idx, n_x, n_dims, le_ct, ge_ct
#         DT_D dy_med, _inc_mult = (1 - (1e-7)), curr_dy
#         np.ndarray[DT_LL_NP_t, ndim=2, mode='c'] mins
#         np.ndarray[DT_D_NP_t, ndim=2, mode='c'] ds, dys, dy_sort
# 
#     n_x = x.shape[0]
#     n_mins = y.shape[0]
